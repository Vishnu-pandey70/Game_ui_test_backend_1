    ...
    useEffect(() => {
        ...
        temp = temp.filter((g) =>
            (g.game_subcategory || []).includes(subcategoryFilter)
        );
        if (providerFilter)
        temp = temp.filter((g) => g.provider_name === providerFilter);

        // Sorting
        if (sortBy === "category") {
        temp.sort((a, b) =>
            getCategoryName(a.game_category).localeCompare(getCategoryName(b.game_category))
        );
        } else if (sortBy === "subcategory") {
        temp.sort((a, b) => ((a.game_subcategory || [])[0] || "").localeCompare((b.game_subcategory || [])[0] || ""));
        }
